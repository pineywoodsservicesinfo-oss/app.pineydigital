...
def _migrate_db():
    ...
    if "pipeline_type" not in columns:
        migrations.append("ALTER TABLE leads ADD COLUMN pipeline_type TEXT DEFAULT 'enterprise'")
    if "parent_company" not in columns:
        migrations.append("ALTER TABLE leads ADD COLUMN parent_company TEXT")
    if "franchise_brand" not in columns:
        migrations.append("ALTER TABLE leads ADD COLUMN franchise_brand TEXT")
    if "locations_count" not in columns:
        migrations.append("ALTER TABLE leads ADD COLUMN locations_count INTEGER DEFAULT 1")
    if "employee_count" not in columns:
        migrations.append("ALTER TABLE leads ADD COLUMN employee_count TEXT")
    if "estimated_revenue" not in columns:
        migrations.append("ALTER TABLE leads ADD COLUMN estimated_revenue TEXT")
    if "tech_stack" not in columns:
        migrations.append("ALTER TABLE leads ADD COLUMN tech_stack TEXT")
    if "growth_signals" not in columns:
        migrations.append("ALTER TABLE leads ADD COLUMN growth_signals TEXT")
    if "decision_makers" not in columns:
        migrations.append("ALTER TABLE leads ADD COLUMN decision_makers TEXT")
    if "lead_source" not in columns:
        migrations.append("ALTER TABLE leads ADD COLUMN lead_source TEXT DEFAULT 'google_maps'")
    ...
    if "channel_detail" not in log_columns:
        migrations.append("ALTER TABLE outreach_log ADD COLUMN channel_detail TEXT")
    if "sequence_step" not in log_columns:
        migrations.append("ALTER TABLE outreach_log ADD COLUMN sequence_step INTEGER DEFAULT 1")
...
def _create_enterprise_tables(conn):
    """Create tables for enterprise pipeline."""
    c = conn.cursor()

    # Decision makers table (multiple contacts per enterprise lead)
    c.execute("""
    ...
    # Outreach sequences table (multi-touch campaign tracking)
    c.execute("""
    ...
    # Campaigns table (outreach campaign definitions)
    c.execute("""
        CREATE TABLE IF NOT EXISTS outreach_campaigns (
            id              INTEGER PRIMARY KEY AUTOINCREMENT,
            name            TEXT NOT NULL,
            pipeline_type   TEXT DEFAULT 'enterprise',
            industry        TEXT,
...
def add_decision_maker(lead_id: int, name: str, title: str = None,
                       email: str = None, phone: str = None,
                       linkedin_url: str = None, is_primary: bool = False) -> int:
    """Add a decision maker to a lead. Returns decision maker ID."""
    conn = get_connection()
    c = conn.cursor()

    c.execute("""
        INSERT INTO decision_makers
            (lead_id, name, title, email, phone, linkedin_url, is_primary)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    """, (lead_id, name, title, email, phone, linkedin_url, 1 if is_primary else 0))
...
def get_decision_makers(lead_id: int) -> list:
    """Get all decision makers for a lead."""
    conn = get_connection()
    c = conn.cursor()
...
def update_decision_maker(dm_id: int, fields: dict):
    """Update decision maker fields."""
    fields["updated_at"] = datetime.now().isoformat()
    fields["id"] = dm_id

    set_clause = ", ".join(f"{k} = ?" for k in fields if k not in ["id", "updated_at"])
    values = [fields[k] for k in fields if k not in ["id", "updated_at"]] + [dm_id]

    conn = get_connection()
    conn.execute(f"UPDATE decision_makers SET {set_clause} WHERE id = ?", values)
    conn.commit()
    conn.close()
...
def create_sequence(lead_id: int, sequence_id: int = 1) -> int:
    """Create an outreach sequence for a lead. Returns sequence ID."""
    conn = get_connection()
    c = conn.cursor()

    c.execute("""
        INSERT INTO outreach_sequences (lead_id, sequence_id, current_step, status)
        VALUES (?, ?, 0, 'active')
    """, (lead_id, sequence_id))
...
def get_sequence(lead_id: int) -> dict | None:
    """Get active sequence for a lead."""
    conn = get_connection()
    c = conn.cursor()
...
def advance_sequence(lead_id: int) -> dict:
    """Advance sequence to next step. Returns next step info or None if complete."""
    conn = get_connection()
    c = conn.cursor()
    ...
    max_step = len(SEQUENCE_TIMING)
    if next_step >= max_step:
        # Sequence complete
        c.execute("""
            UPDATE outreach_sequences
            SET status = 'completed', last_sent_at = datetime('now')
            WHERE id = ?
        """, (seq["id"],))
        conn.commit()
        conn.close()
        return {"status": "completed", "step": next_step}

    # Advance to next step
    c.execute("""
        UPDATE outreach_sequences
        SET current_step = ?, last_sent_at = datetime('now')
        WHERE id = ?
    """, (next_step, seq["id"]))
    ...
    return {"status": "active", "step": next_step}


def pause_sequence(lead_id: int, reason: str = None):
...
def get_pending_sequences(limit: int = 100) -> list:
    """Get leads ready for their next sequence step."""
    conn = get_connection()
    c = conn.cursor()

    c.execute("""
        SELECT s.*, l.business_name, l.email, l.owner_email
...
def create_campaign(name: str, pipeline_type: str = 'enterprise',
                    industry: str = None, sequence_config: str = None) -> int:
    """Create an outreach campaign. Returns campaign ID."""
    conn = get_connection()
    c = conn.cursor()

    c.execute("""
        INSERT INTO outreach_campaigns
            (name, pipeline_type, industry, sequence_config)
        VALUES (?, ?, ?, ?)
    """, (name, pipeline_type, industry, sequence_config))
...
def get_campaigns(active_only: bool = True) -> list:
    """Get all campaigns."""
    conn = get_connection()
    c = conn.cursor()
    ...
    if active_only:
        ...
        c.execute("SELECT * FROM outreach_campaigns ORDER BY created_at DESC")
